					...
					if (type==kind.trim()) {
						for (var island=0;island<window.drops[type].length;++island) {
							for (var stage in window.drops[type][island]) {
								// alert(stage);
								if (stage == 'name' || stage == 'shortName' || stage == 'day' || stage == 'global' || stage == 'condition' || stage == 'completion' || stage == 'challenge' || stage == 'challengeData' || stage == 'showManual' || stage == 'gamewith' || stage == 'teamDatabase') continue;
								if (window.drops[type][island][stage] = id || window.drops[type][island][stage].indexOf(id) != -1) {
									return true;									
								}	
							}						
						}
